...
class  MenuManager:
    ...
    def export(self):
       if not self.manager.is_logged_in():
        print("Ошибка: Вы не вошли в систему")
        return
    ...
    def _export_csv(self):
      ...
      cursor.execute("""
      SELECT website, website_url, username, encrypted_password, category, notes, created_at
      ...
      if not rows:
        print("Нет паролей для экспорта")
        return
      ...
      with open(filename, 'w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["Сайт", "URL", "Логин", "Пароль", "Категория", "Заметки", "Дата создания"])
        
        for row in rows:
            website, url, username, enc_password, category, notes, created_at = row
            encrypted = base64.b64decode(enc_password)
            decrypted = self.password_manager.cipher.decrypt(encrypted).decode()
            
            writer.writerow([website, url or "", username, decrypted, category, notes or "", created_at])
    
      print(f"Экспортировано {len(rows)} паролей в {filename}")
    ...
    def _export_json(self):
      ...
      print(f"Экспортировано {len(data)} паролей в {filename}")

    def edit_password_simple(self, old_website, new_website, new_username, new_password):
     try:
        conn = sqlite3.connect(self.db)
        cursor = conn.cursor()
        
        # Шифруем новый пароль
        encrypted = self.password_manager.cipher.encrypt(new_password.encode())
        ...
        """, (new_website, new_username, encrypted_b64, old_website, self.password_manager.user_id))
    ...
    def edit_password(self):
     ...
     if not self.password_manager:
        print("Ошибка: система не загружена")
        return
     ...
     old_website = input("\nВведите название сайта, который хотите изменить: ").strip()
     if not old_website:
        print("Название не может быть пустым")
        return
    
     print("\nВведите новые данные:")
     new_website = input("Новый сайт: ").strip()
     new_username = input("Новый логин: ").strip()
     ...
     confirm = input(f"\nСохранить изменения для {old_website}? (да/нет): ").strip().lower()
     if confirm == "да":
        self.edit_password_simple(old_website, new_website, new_username, new_password)
    ...
    def show_menu(self):
        ...
        print("4 - Экспорт паролей в CSV/JSON")
        print("5 - Изменение информации логина")
        print("6 - Выход")
        ...
        while True:
            ...
            if choice == "1":
                ...
                print("Переход в функцию Экспорт паролей CSV/JSON")
                time.sleep(2)
                self.export()
            elif choice == "5":
               print("Переход в функцию Изменение информации логина")
               time.sleep(2)
               self.edit_password()
            elif choice == "6":
                print("Выхожу...")
